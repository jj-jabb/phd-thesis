...
/**
 * Created by IntelliJ IDEA. User: bennett Date: Nov 20, 2005 Time: 9:28:30 AM
 * To change this template use Options | File Templates.
 * ************************************ Magnetic Resonance in Medicine Final
 * Project Released: December 1, 2005
 * 
 * class Triangle Represent a triangle
 * 
...
public class TriangleSigned {
	...
	// represent the distance from the origin to the plane of the triangle along
	// the normal
	/** The plane offset. */
	 ...
	 *            the c
	 */
	public TriangleSigned(PT a, PT b, PT c) {
		pts = new PT[3];
		pts[0] = a;
		pts[1] = b;
		pts[2] = c;
	 ...
	 * int vol0, vol1, vol2;
	 * 
	 * vol0 = volumeSign(a,pts[0],pts[1],b); vol1 =
	 * volumeSign(a,pts[1],pts[2],b); vol2 = volumeSign(a,pts[2],pts[0],b);
	 * 
	 * // same sign: segment intersects interior of triangle
	 * if(((vol0>0)&&(vol1>0)&&(vol2>0))||((vol0<0)&&(vol1<0)&&(vol2<0))) return
	 * (vol0);
	 * 
	 * // oposite sign: segment does not intersect the interior if(((vol0>0) ||
	 * (vol1>0) || (vol2>0)) && ((vol0<0) || (vol1<0) || (vol2<0))) return 0;
	 * else if((vol0==0) && (vol1==0) && (vol2==0)) { // coplanar float dist0 =
	 * SegSegIntersect(a,b,pts[0],pts[1]); float dist1 =
	 * SegSegIntersect(a,b,pts[1],pts[2]); float dist2 =
	 * SegSegIntersect(a,b,pts[0],pts[2]); PT r = b.minus(a);
	 * if((dist0<=dist1)&&(dist0<=dist2) && (dist0<=1)) { PT pt =
	 * a.plus(r.times(dist0)); if(pt.equals(pts[0]) || pt.equals(pts[1])) throw
	 * new DegenerateIntersectionException("vertex intersection"); if(dist0==0
	 * || dist0==1) throw new
	 * DegenerateIntersectionException("edge intersection"); return 1; }
	 * if(dist1<=dist2 && dist1<=1) { PT pt = a.plus(r.times(dist1));
	 * if(pt.equals(pts[1]) || pt.equals(pts[1])) throw new
	 * DegenerateIntersectionException("vertex intersection"); if(dist0==0 ||
	 * dist0==1) throw new DegenerateIntersectionException("edge intersection");
	 * return 1; } if(dist2<=1) { PT pt = a.plus(r.times(dist2));
	 * if(pt.equals(pts[0]) || pt.equals(pts[2])) throw new
	 * DegenerateIntersectionException("vertex intersection"); if(dist0==0 ||
	 * dist0==1) throw new DegenerateIntersectionException("edge intersection");
	 * return 1; } throw new DegenerateIntersectionException(
	 * "Error in Triangle.Intersect: Invalid result."); }
	 * 
	 * //* two zeros: segment intersects vertex if( ((vol0==0) && (vol1==0)) ||
	 * ((vol0==0) && (vol2==0)) || ((vol1==0) && (vol2==0))) throw new
	 * DegenerateIntersectionException("vertex intersection");
	 * 
	 * //* one zero: segment intersects edge * if( ((vol0==0) ) || ((vol1==0))
	 * || ( (vol2==0))) throw new
	 * DegenerateIntersectionException("vertex intersection");
	 * 
	 * throw new
	 * DegenerateIntersectionException("Error in Triangle.Intersect: Case failure."
	 * ); }
	 */
	 ...
	 *             the degenerate intersection exception
	 */
	...
	/*
	 * Assumption: p lies in the plane containing T. Returns a char: 'V': the
	 * query point p coincides with a Vertex of triangle T. 'E': the query point
	 * p is in the relative interior of an Edge of triangle T. 'F': the query
	 * point p is in the relative interior of a Face of triangle T. '0': the
	 * query point p does not intersect (misses) triangle T.
	 */
	...
	char SegTriCross(PT q, PT r) {
		...
		} else if (((vol0 == 0) && (vol1 == 0)) || ((vol0 == 0) && (vol2 == 0))
				|| ((vol1 == 0) && (vol2 == 0))) {
			return 'v';
		} else if ((vol0 == 0) || (vol1 == 0) || (vol2 == 0)) {
			return 'e';
	...
	public int volumeSign(PT a, PT b, PT c, PT d) {
		...
		cy = c.y - d.y;
		cz = c.z - d.z;
		float vol = ax * (by * cz - bz * cy) + ay * (bz * cx - bx * cz) + az
				* (bx * cy - by * cx);
		if (vol > 0) {
			return 1;
		}
	 ...
	 *             the degenerate intersection exception
	 */
	public IntersectResult findIntersect(PT a, PT b)
			throws DegenerateIntersectionException {
		PT r = b.minus(a);
		float denom = r.dot(planeNormal);
		if (denom == 0) {
			if (this.planeOffset != planeNormal.dot(a)) {
				throw new DegenerateIntersectionException(
						"No intersection in plane");
			}
			...
			if ((dist0 < dist1) && (dist0 < dist2)) {
				return new IntersectResult(a.plus(r.times(dist0)), dist0,
						planeNormal);
			}
			if (dist1 < dist2) {
				return new IntersectResult(a.plus(r.times(dist1)), dist1,
						planeNormal);
			}
			return new IntersectResult(a.plus(r.times(dist2)), dist2,
					planeNormal);
		}
		float t = (planeOffset - a.dot(planeNormal)) / denom;
		if ((t < 0) || (t > 1)) {
			throw new DegenerateIntersectionException(
					"Error in findIntersect: No Intersection");
		}

		PT planeIntersect = a.plus(r.times(t));
		if (planeNormal.z != 0) {
			planeIntersect.z = (planeOffset - planeIntersect.x * planeNormal.x - planeNormal.y
					* planeIntersect.y)
					/ planeNormal.z;
		} else {
			if (planeNormal.y != 0) {
				planeIntersect.y = (planeOffset - planeIntersect.x
						* planeNormal.x - planeNormal.z * planeIntersect.z)
						/ planeNormal.y;
			} else {
				planeIntersect.x = (planeOffset - planeIntersect.y
						* planeNormal.y - planeNormal.z * planeIntersect.z)
						/ planeNormal.x;
			}
		}
	 ...
	 * P1 + mua (P2 - P1) Pb = P3 + mub (P4 - P3) Returns fractional distance
	 * from p1 to p2 where it intersection p3 to p4
	 */
	/**
	 * Seg seg intersect.
	 * 
	...
	public boolean contains(PT p) {
		return (sameside(p, pts[0], pts[1], pts[2])
				&& sameside(p, pts[1], pts[2], pts[0]) && sameside(p, pts[2],
				pts[0], pts[1]));
	}
	...
	static boolean InTri2D(PT Tp[], PT pp) {
		...
		/*
		 * compute three AreaSign() values for pp w.r.t. each edge of the face
		 * in 2D
		 */
		...
		if ((area0 == 0) && (area1 > 0) && (area2 > 0) || (area1 == 0)
				&& (area0 > 0) && (area2 > 0) || (area2 == 0) && (area0 > 0)
				&& (area1 > 0)) {
			return true;
		}
		...
		if ((area0 > 0) && (area1 > 0) && (area2 > 0) || (area0 < 0)
				&& (area1 < 0) && (area2 < 0)) {
			return true;
		}
		...
		if ((area0 == 0) && (area1 == 0) || (area0 == 0) && (area2 == 0)
				|| (area1 == 0) && (area2 == 0)) {
			return true;
		} else {
			return false;
