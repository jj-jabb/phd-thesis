 ...
 */
public class VisualizationMOGAC3D extends VisualizationTriangleMesh implements
	...
	/** The active contour. */
	protected MOGAC3D activeContour;
	/** The count cycle. */
	...
	/** The renderer. */
	protected MOGACRenderer3D renderer;
	 ...
	 *            the active contour
	 */
	public VisualizationMOGAC3D(int width, int height, MOGAC3D activeContour) {
		super(width, height);
		pose = new Matrix4f();
		pose.setIdentity();
	...
	public void createVisualizationParameters(
		...
		centerParam.setValue(center = new Point3f(0.5f * activeContour.rows,
				0.5f * activeContour.cols, 0.5f * activeContour.slices));
	}
	...
	public void mouseDragged() {
		...
		if (requestUpdate) {
			...
			renderer.setTargetCenter(new Point3f(tx / width + 2 * center.x
					/ activeContour.rows - 1, -ty / height + 2 * center.y
					/ activeContour.cols - 1, 2 * center.z
					/ activeContour.slices - 1));
		}
	}
	...
	public void update(ParamModel model, ParamInputView view) {
		...
		renderer.setTargetCenter(new Point3f(tx / width + 2 * center.x
				/ activeContour.rows - 1, -ty / height + 2 * center.y
				/ activeContour.cols - 1, 2 * center.z / activeContour.slices
				- 1));
