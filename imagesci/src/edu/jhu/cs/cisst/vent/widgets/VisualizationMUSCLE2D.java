...
import edu.jhu.cs.cisst.vent.renderer.processing.MUSCLERenderer2D;
 ...
 */
public class VisualizationMUSCLE2D extends
		VisualizationImage2D implements PlayPauseStopEventListener {
				...
				for (time = 0; time < simulationDuration && running; time++) {
					if (!pause) {
						...
						if (captureParam.getValue()
								&& time % simulator.getResamplingInterval() == 0) {
							screenshot = null;
	...
	/** The simulator. */
	protected MuscleActiveContour2D simulator;
	 ...
	 */
	public VisualizationMUSCLE2D(int width, int height,
			MuscleActiveContour2D simulator) {
	...
	public ParamCollection create() {
		...
		levelsetRenderer = new MUSCLERenderer2D(this, simulator);
		...
		if (running && captureParam.getValue()) {
			if (requestVideoFrame) {
				...
				String f = (new File(outputFileParam.getValue(), String.format(
						"screenshot_%05d.png", t - 1))).getAbsolutePath();
