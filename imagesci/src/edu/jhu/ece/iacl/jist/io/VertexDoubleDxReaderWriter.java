...
/**
 * The Class VertexDoubleDxReaderWriter.
 */
public class VertexDoubleDxReaderWriter extends FileReaderWriter<Point3d[]> {

	/** The extension filter. */
	protected FileExtensionFilter extensionFilter;

	/**
	 * Instantiates a new vertex double dx reader writer.
	 */
	public VertexDoubleDxReaderWriter() {
		super(new FileExtensionFilter(new String[] { "dx" }));
	}

	/** The Constant readerWriter. */
	protected static final VertexDoubleDxReaderWriter readerWriter = new VertexDoubleDxReaderWriter();

	/**
	 * Gets the single instance of VertexDoubleDxReaderWriter.
	 * 
	 * @return single instance of VertexDoubleDxReaderWriter
	 */
	public static VertexDoubleDxReaderWriter getInstance() {
		return readerWriter;
	}
	 ...
	 */
	@Override
	protected Point3d[] readObject(File f) {
		BufferedReader in;
		...
		if (m.find()) {
			...
			for (int i = 1; i < strs.length - 1; i += 3) {
				try {
					...
					p.x = Double.parseDouble(strs[i]);
					p.y = Double.parseDouble(strs[i + 1]);
					p.z = Double.parseDouble(strs[i + 2]);
					points[(i - 1) / 3] = p;
	 ...
	 */
	@Override
	protected File writeObject(Point3d[] results, File f) {
		try {
